...
console.log("Semaphore Agent ECS Scaler Lambda Function Starting...");
...
const agentTypeToken = process.env.SEMAPHORE_AGENT_TOKEN;
if (!agentTokenParameterName) {
  console.error("No SEMAPHORE_AGENT_TOKEN specified.")
  return {
    statusCode: 500,
    message: "error",
  };
...
const clusterName = process.env.SEMAPHORE_AGENT_ECS_CLUSTER_NAME;
if (!clusterName) {
  console.error("No SEMAPHORE_AGENT_ECS_CLUSTER_NAME specified.")
  return {
    statusCode: 500,
    message: "error",
  };
...
const serviceName = process.env.SEMAPHORE_AGENT_ECS_SERVICE_NAME;
if (!serviceName) {
  console.error("No SEMAPHORE_AGENT_ECS_SERVICE_NAME specified.")
  return {
    statusCode: 500,
    message: "error",
  };
...
try {
  while (true) {
    tick(agentTypeToken, stackName, clusterName, serviceName, ecsClient, appAutoScalingClient, cloudwatchClient, semaphoreEndpoint, overprovisionStrategy, overprovisionFactor);
